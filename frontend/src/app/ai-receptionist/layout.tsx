...
export default function AIReceptionistLayout({
  ...
  const navItems = [
    { name: "Live Conversation", href: "/ai-receptionist", icon: Bot },
    { name: "Knowledge Base", href: "/ai-receptionist/knowledge-base", icon: BookOpen },
    { name: "AI Lead Pipeline", href: "/ai-receptionist/leads", icon: PhoneCall },
  ];
            ...
            <nav className="p-3.5 space-y-1.5 overflow-y-auto flex-1 custom-scrollbar">
              ...
              <div className="pt-4 mt-4 border-t border-slate-800/80">
                <Link
                  ...
                  className="flex items-center gap-2.5 px-3.5 py-2 text-[12px] font-semibold text-cyan-400 hover:text-white hover:bg-cyan-500/10 rounded-xl transition-all border border-cyan-500/20 bg-cyan-500/5 shadow-xs"
                >
                  <ArrowLeft className="w-4 h-4 shrink-0" />
                  <span className="truncate">{returnRoute.label}</span>
                </Link>
              </div>
            </nav>
          </div>
            ...
            <div className="bg-[#111C33]/80 border border-slate-800 rounded-xl p-3 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-[12px] font-bold text-white truncate">
                  LuckNexa AI Concierge
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
                  <span className="text-[10px] text-slate-400 font-medium">Neural Engine Active</span>
                </div>
              </div>
            </div>
          </div>
