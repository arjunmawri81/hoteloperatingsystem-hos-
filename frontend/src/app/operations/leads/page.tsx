...
import { Plus, X, Search, PhoneCall, Bot, CheckCircle2, RefreshCw, Building2 } from "lucide-react";
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [leadsRes, hotelsRes] = await Promise.all([
        leadsApi.getAll({ hotelId: targetHotel }),
  ...
  const handleCreateLead = async (e: React.FormEvent) => {
    ...
    try {
      const created = await leadsApi.create({
        ...
        stage: newLead.stage,
        aiSummary: newLead.aiSummary || "Captured via Front Desk Sales CRM",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[26px] font-bold text-[#111827] tracking-tight">
            Sales &amp; Leads Management (CRM)
          </h1>
          <p className="text-[13px] text-[#6B7280] mt-0.5">
            Autonomous inbound/outbound AI qualification, voice call summaries &amp; sales conversion
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="flex items-center gap-1.5 bg-white border border-[#D1D5DB] px-3 py-1.5 rounded text-[13px] shadow-xs">
            ...
            <span className="text-[#111827] font-semibold text-[13px]">
              {hotels.find((h) => h.id === (user?.hotelId || currentHotelId))?.name || user?.hotelName || "Property Operations"}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-lg border border-[#E5E7EB] shadow-xs">
          ...
          <div className="text-[24px] font-bold text-purple-700 mt-1.5">{aiQualifiedCount} Leads</div>
        </div>
        <div className="bg-white p-5 rounded-lg border border-[#E5E7EB] shadow-xs">
          <div className="text-[11px] font-bold text-blue-600 uppercase">Active Enquiries</div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-10 text-center text-[#9CA3AF]">
                    No sales leads in database yet. Click &quot;Capture Lead&quot; to log a new inquiry.
                  </td>
                </tr>
              ) : (
            ...
            <form onSubmit={handleCreateLead} className="space-y-4 text-[13px]">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Corporate Offsite · 15 Rooms for 3 Nights"
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="e.g. AI Calling Agent confirmed event dates and attendee count. Awaiting AV quote."
